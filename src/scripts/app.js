...
var imgsScore = document.querySelectorAll(
  ".caroussel1__picture img.caroussel__picture__img"
);

let mangaPopular = function () {
  fetch("https://api.jikan.moe/v4/manga?limit=8&order_by=score&sort=desc")
...
var imgsRecommendation = document.querySelectorAll(
  ".caroussel3__picture img.caroussel__picture__img"
);

let mangaRecommendation = function () {
  fetch("https://api.jikan.moe/v4/recommendations/manga?&limit=8")
    .then(function (response) {
      return response.json();
    })
    .then(function (manga) {
      for (let i = 0; i < imgsScore.length; i++) {
        ...
        imgsRecommendation[i].src =
          manga.data[i].entry[0].images.webp.image_url;
      }
    });
};
